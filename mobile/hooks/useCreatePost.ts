...
export const useCreatePost = () => {
  ...
  const createPostMutatuion = useMutation({
  ...
  const handleImagePicker = async (useCamera: boolean = false) => {
    ...
    if (permissionResult.status !== "granted") {
      ...
      Alert.alert(
        "Permission Denied",
        `You need to grant permission to access ${source}`
      );
      return;
    }
  ...
  const createPost = () => {
    if (!content.trim() && !selectedImage) {
      Alert.alert(
        "Empty Post",
        "Please write something or select an image before posting."
      );
      return;
    }
    ...
    createPostMutatuion.mutate(postData);
  };
  ...
  return {
    ...
    setContent,
    selectedImage,
    isCreating: createPostMutatuion.isPending,
